...
print(lidar_folder)
# imageset = os.path.join(rootdir, 'ImageSets', 'train.txt')

lidars = sorted(os.listdir(lidar_folder))
print(lidars)

# remove file without labels
# lidars = []
...
for l in lidars:
    ...
    #print(pts.dtype)
    outdir = rootdir + "/" + outfile
    if not os.path.isdir(outdir):
        os.mkdir(outdir)
    # pts.tofile(rootdir+"/lidar.bin/{}.bin".format(frame))
    pts.tofile(outdir + "/{}.bin".format(frame))
