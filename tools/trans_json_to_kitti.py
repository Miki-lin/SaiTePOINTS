...
def parse_one_detection_obj(m, d):
    txt_label = [0.0]*16
    txt_label[0] = d['obj_type']

    txt_label[8] = d['psr']['scale']['z']
    txt_label[9] = d['psr']['scale']['y']
    txt_label[10] = d['psr']['scale']['x']

    pos_x = d['psr']['position']['x']
    pos_y = d['psr']['position']['y']
    pos_z = d['psr']['position']['z']-d['psr']['scale']['z']/2
    pos_json = np.array([float(pos_x), float(pos_y), float(pos_z), 1]).T
    pos_txt = np.matmul(m, pos_json)
    txt_label[11] = pos_txt[0]
    txt_label[12] = pos_txt[1]
    txt_label[13] = pos_txt[2]

    txt_label[14] = -math.pi/2 - float(d['psr']['rotation']['z'])
...
def trans_label_to_kitti(src_label, src_calib, tgt_label):
    files = os.listdir(src_label)
    files.sort()
    for file in files:
        frame, _ = os.path.splitext(file)
        print(frame)
        m = get_detection_inv_matrix(src_calib, frame)
        print(m)
        # m = None

        with open(os.path.join(src_label, file)) as f:
            data = json.load(f)
            for obj in data:
                txt_obj = parse_one_detection_obj(m, obj)
                print(txt_obj)
                with open(os.path.join(tgt_label, frame + '.txt'), 'a') as f:
                    f.write(' '.join(str(i) for i in txt_obj) + '\n')


def parse_without_calib(d):
    txt_label = [0.0]*15
    txt_label[0] = str(d['obj_type'])

    txt_label[8] = float(d['psr']['scale']['z'])
    txt_label[9] = float(d['psr']['scale']['y'])
    txt_label[10] = float(d['psr']['scale']['x'])

    txt_label[11] = float(d['psr']['position']['x'])
    txt_label[12] = float(d['psr']['position']['y'])
    txt_label[13] = float(d['psr']['position']['z']) - float(d['psr']['scale']['z'])/2   # json to kitti_txt

    txt_label[14] = float(-math.pi/2 - float(d['psr']['rotation']['z']))

    return txt_label


def trans_kitti_without_calib(src_label, tgt_label):
    files = os.listdir(src_label)
    files.sort()
    for file in files:
        frame, _ = os.path.splitext(file)
        ...
        with open(os.path.join(src_label, file)) as f:
            data = json.load(f)
            # 添加空的txt
            # with open(os.path.join(tgt_label, frame + '.txt'), 'w') as f:
            #     f.write('')
            for obj in data:
                txt_obj = parse_without_calib(obj)
                print(txt_obj)
                with open(os.path.join(tgt_label, frame + '.txt'), 'a') as f:
                    f.write(' '.join(str(i) for i in txt_obj) + '\n')
...
if __name__ == "__main__":
    # src_label = '../data/test/label2'
    # src_calib = '../data/test/calib'
    # tgt_label = '../data/test/label3'
    # if not os.path.exists(tgt_label):
    #     os.mkdir(tgt_label)
    # trans_label_to_kitti(src_label, src_calib, tgt_label)

    src_label = '../data/haizhu_1134/label'
    tgt_label = '../data/haizhu_1134/label_2'
    if not os.path.exists(tgt_label):
        os.mkdir(tgt_label)
    trans_kitti_without_calib(src_label, tgt_label)